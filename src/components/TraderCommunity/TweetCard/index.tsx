...
import ModalTweetDetail from '../ModalTweetDetail'
...
const TweetCard = ({ username, time, fullname, content, avatar, isLiked, isReplies, isBanner }: Props) => {
  const [tweetDetailOpen, setOpenTweetDetail] = useState(false)

  const tweetContainerRef = useRef<any>(null)

  const handleOpenTweetDetail = (event: any) => {
    if (event.currentTarget.getAttribute('data-disable-button')) {
      return null
    } else {
      return setOpenTweetDetail(true)
    }
  }
  ...
  return (
    <>
      <article
        ...
        className="bg-white dark:bg-surface-400 rounded-xl hover:cursor-pointer"
      >
        {isBanner && (
          <Banner/>
        )}
      ...
      <ModalTweetDetail isOpen={tweetDetailOpen} onClose={() => setOpenTweetDetail(false)} />
    </>
  )
}
